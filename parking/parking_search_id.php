...
if (isJson($data)) {
    ...
    $parking_search_id = $decodeData['parking_id'];
    $parking_search_list = array();

    $query = "SELECT * FROM parking_lot WHERE id = '$parking_search_id' ";
    $stmt = $conn->prepare($query);

    $stmt->bindParam('id', $parking_search_id, PDO::PARAM_STR);
    ...
    if ($count > 0) {
        ...
        $json_value2 = array(
            'statusCode' => '200',
            'description' => 'Parking ID Data Found',
        ...
        $json_value2 = array(
            'statusCode' => '400',
            'description' => 'No Data Found for Parking ID Search.',
            'Zip Search List' => $parking_search_list,
